...
export function generateInvoicePDF(data: InvoicePDFData, org?: OrganizationInfo): jsPDF {
  const doc = new jsPDF();
  ...
  const companyContact = org?.email || org?.phone || "";
  ...
  if (companyContact) {
    doc.text(companyContact, pageWidth - 20, 55, { align: "right" });
  }
