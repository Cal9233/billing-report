...
describe("Reports GET /api/reports", () => {


  it("returns valid report structure with all required fields", async () => {
    const mockInvoices = [
      {
        id: "inv-1",
    ...
    expect(body).toHaveProperty("invoiceSummary");
    expect(body).toHaveProperty("poSummary");
    expect(body).toHaveProperty("monthlyData");
    expect(body).toHaveProperty("customerRevenue");
    expect(body).toHaveProperty("recentInvoices");
    expect(body).toHaveProperty("recentPOs");
    expect(body).toHaveProperty("customerCount");
  });

  it("customerRevenue groups by customerId — two customers with same name get separate entries (M3 fix)", async () => {
    const mockInvoices = [
      {
        id: "inv-a",
        invoiceNumber: "INV-A",
        ...
        customer: { id: "cust-1", name: "Acme Corp" }, // id=cust-1
        lineItems: [],
        createdAt: new Date(),
      },
      {
    ...
    // With M3 fix, both customers should appear separately in customerRevenue
    expect(body.customerRevenue).toHaveLength(2);
    const ids = body.customerRevenue.map((c: { id: string }) => c.id);
    expect(ids).toContain("cust-1");
    expect(ids).toContain("cust-2");

    // Revenue correctly attributed per id
    const cust1 = body.customerRevenue.find((c: { id: string }) => c.id === "cust-1");
    const cust2 = body.customerRevenue.find((c: { id: string }) => c.id === "cust-2");
    expect(cust1.totalRevenue).toBe(800);
    expect(cust2.totalRevenue).toBe(400);
  });

  it("report with no data returns zero-filled summary", async () => {
    mockInvoiceFindMany.mockResolvedValueOnce([]);
    mockPOFindMany.mockResolvedValueOnce([]);
    mockCustomerFindMany.mockResolvedValueOnce([]);
    ...
    expect(body.invoiceSummary.total).toBe(0);
    expect(body.invoiceSummary.totalAmount).toBe(0);
    expect(body.poSummary.total).toBe(0);
    expect(body.customerRevenue).toHaveLength(0);
    expect(body.customerCount).toBe(0);
  });

  it("invoiceSummary.byStatus correctly categorises all statuses", async () => {
    const statuses = ["draft", "sent", "paid", "overdue", "cancelled"];
    const mockInvoices = statuses.map((status, i) => ({
      id: `inv-${i}`,
    ...
    const { byStatus } = body.invoiceSummary;
  ...
  it("paidAmount only includes paid invoices", async () => {
    const mockInvoices = [
      { id: "i1", status: "paid", total: 500, issueDate: new Date(), customer: { id: "c1", name: "X" }, lineItems: [], createdAt: new Date(), invoiceNumber: "INV-1" },
      { id: "i2", status: "sent", total: 300, issueDate: new Date(), customer: { id: "c1", name: "X" }, lineItems: [], createdAt: new Date(), invoiceNumber: "INV-2" },
      { id: "i3", status: "overdue", total: 200, issueDate: new Date(), customer: { id: "c1", name: "X" }, lineItems: [], createdAt: new Date(), invoiceNumber: "INV-3" },
    ];

    mockInvoiceFindMany.mockResolvedValueOnce(mockInvoices);
    mockPOFindMany.mockResolvedValueOnce([]);
    mockCustomerFindMany.mockResolvedValueOnce([{ id: "c1", name: "X", _count: { invoices: 3, purchaseOrders: 0 } }]);

    const res = await reportGET();
    const body = await res.json();

    expect(body.invoiceSummary.paidAmount).toBe(500);
    expect(body.invoiceSummary.outstandingAmount).toBe(500); // sent(300) + overdue(200)
  });

  it("monthlyData array has exactly 12 entries for last 12 months", async () => {
    mockInvoiceFindMany.mockResolvedValueOnce([]);
    mockPOFindMany.mockResolvedValueOnce([]);
    mockCustomerFindMany.mockResolvedValueOnce([]);
  ...
  it("recentInvoices returns at most 5 entries", async () => {
    const tenInvoices = Array.from({ length: 10 }, (_, i) => ({
      id: `inv-${i}`,
      invoiceNumber: `INV-0${i}`,
      status: "draft",
      total: 100,
      issueDate: new Date("2026-03-01"),
      customer: { id: "c1", name: "Foo" },
      lineItems: [],
      createdAt: new Date(),
    }));

    mockInvoiceFindMany.mockResolvedValueOnce(tenInvoices);
    mockPOFindMany.mockResolvedValueOnce([]);
    mockCustomerFindMany.mockResolvedValueOnce([]);
  ...
  it("customerRevenue capped at 10 entries (top-10)", async () => {
    const fifteenCustomers = Array.from({ length: 15 }, (_, i) => ({
      id: `cust-${i}`,
      name: `Corp ${i}`,
      _count: { invoices: 1, purchaseOrders: 0 },
    }));
    const fifteenInvoices = Array.from({ length: 15 }, (_, i) => ({
      id: `inv-${i}`,
      invoiceNumber: `INV-${i}`,
      status: "paid",
      total: (i + 1) * 100,
      issueDate: new Date("2026-03-01"),
      customer: { id: `cust-${i}`, name: `Corp ${i}` },
      lineItems: [],
      createdAt: new Date(),
    }));

    mockInvoiceFindMany.mockResolvedValueOnce(fifteenInvoices);
    mockPOFindMany.mockResolvedValueOnce([]);
    mockCustomerFindMany.mockResolvedValueOnce(fifteenCustomers);

    const res = await reportGET();
    const body = await res.json();
    expect(body.customerRevenue.length).toBeLessThanOrEqual(10);
  });
});
